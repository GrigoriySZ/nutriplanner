...
SESSION_KET = 'meal_plan'

class MealPlanSession:
    def __init__(self, request):
        self.session = request.session
        meal_plan = self.session.get(SESSION_KET)

        if not meal_plan:
            meal_plan = self.session[SESSION_KET] = {}
    ...
    def clear(self):
        self.session[SESSION_KET] = {}
        self.save()

    def save(self):
        self.session.modified = True

    def get_item_and_totals(self):
